...
use Monolog\Logger;
use KED\Middleware\AdminNavigationMiddleware;
use KED\Middleware\CartInitMiddleware;
use KED\Middleware\CustomerAuthenticateMiddleware;
use KED\Middleware\PromiseWaiterMiddleware;
use KED\Middleware\SaveCartMiddleware;
use KED\Middleware\UserAuthenticateMiddleware;
...
use KED\Services\Event\EventDispatcher;
use KED\Services\Helmet;
use KED\Services\Http\Request;
use KED\Services\Http\Response;
use KED\Middleware\ConfigMiddleware;
use KED\Middleware\SessionMiddleware;
use KED\Middleware\RoutingMiddleware;
use KED\Middleware\HandlerMiddleware;
use KED\Middleware\AdminLayoutMiddleware;
use KED\Middleware\FrontLayoutMiddleware;
use KED\Middleware\AlertMiddleware;
use KED\Middleware\ResponseMiddleware;
use KED\Services\MiddlewareManager;
use KED\Services\PromiseWaiter;
use KED\Services\Routing\RouteParser;
...
class App
{
    ...
    protected function prepareToStart()
    {
        ...
        if (!strpos($_SERVER['REQUEST_URI'], 'install') and !file_exists(CONFIG_PATH . DS . 'config.php')) {
            $redirect = new \Symfony\Component\HttpFoundation\RedirectResponse(\KED\get_base_url() . '/install');
