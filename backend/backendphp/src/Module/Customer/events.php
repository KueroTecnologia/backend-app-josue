...
$eventDispatcher->addListener(
    ...
    function (\KED\Services\MiddlewareManager $middlewareManager) {
        $middlewareManager->registerMiddleware(\KED\Module\Customer\Middleware\HeaderIconMiddleware::class, 51);
    },
    5
);

$eventDispatcher->addListener('register.checkout.index.middleware', function (\KED\Services\MiddlewareManager $mm) {
    $mm->registerMiddleware(\KED\Module\Customer\Middleware\Checkout\AccountMiddleware::class, 1);
});
...
$eventDispatcher->addListener(
    ...
    function (&$fields, Container $container) {
        $fields += [
            'customerCollection' => [
                'type' => $container->get(\KED\Module\Customer\Services\Type\CustomerCollectionType::class),
                ...
                'resolve' => function ($rootValue, $args, Container $container, ResolveInfo $info) {
                    if ($container->get(\KED\Services\Http\Request::class)->isAdmin() == false)
                        return [];
                    $collection = new \KED\Module\Customer\Services\CustomerCollection($container);
        ...
        $fields['customerGroups'] = [
            'type' => Type::listOf($container->get(\KED\Module\Customer\Services\Type\CustomerGroupType::class)),
            ...
            'resolve' => function ($rootValue, $args, Container $container, ResolveInfo $info) {
                ...
                if ($container->get(Request::class)->isAdmin() == false)
                    ...
                    return \KED\_mysql()->getTable('customer_group')->where('customer_group_id', '<', 999)->fetchAllAssoc();
            }
        ];

        $fields['customer'] = [
            'type' => $container->get(\KED\Module\Customer\Services\Type\CustomerType::class),
            ...
            'resolve' => function ($rootValue, $args, Container $container, ResolveInfo $info) {
                ...
                if (
                    ...
                    return \KED\_mysql()->getTable('customer')->load($args['id']);
            }
        ];

        $fields['customerAddresses'] = [
            'type' => Type::listOf($container->get(\KED\Module\Customer\Services\Type\AddressType::class)),
            ...
            'resolve' => function ($rootValue, $args, Container $container, ResolveInfo $info) {
                ...
                if (
                    ...
                    return [];
                return \KED\_mysql()->getTable('customer_address')->where('customer_id', '=', $args['customerId'])->fetchAllAssoc();
            }
        ];
    },
    5
...
$eventDispatcher->addListener(
    ...
    function (&$fields, Container $container) {
        $fields['createCustomerAddress'] = [
            'args' => [
                'address' => [
                    'type' => $container->get(\KED\Module\Customer\Services\Type\AddressInputType::class)
                ],
                'customerId' => Type::nonNull(Type::int())
            ],
            'type' => new ObjectType([
    ...
    function (array $items) {
        return array_merge($items, [
            ...
            [
                ...
                "sort_order" => 10,
                "url" => \KED\generate_url("customer.grid"),
                "title" => "Customers",
                "icon" => "users",
                "parent_id" => "customer"
            ]
