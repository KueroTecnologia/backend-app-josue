...
use KED\Module\Catalog\Services\Type\ProductImageType;
use KED\Module\Catalog\Middleware\Widget\FeaturedProduct\FeaturedProductWidgetMiddleware;
use KED\Module\Catalog\Middleware\Widget\ProductFilter\ProductFilterWidgetMiddleware;
use KED\Module\Catalog\Middleware\Product\View\InitMiddleware as ProductInitMiddleware;
use KED\Module\Catalog\Middleware\Category\View\InitMiddleware as CategoryInitMiddleware;
...
$eventDispatcher->addListener('register.widget.create.middleware', function (\KED\Services\MiddlewareManager $mm) {
    $mm->registerMiddleware(\KED\Module\Catalog\Middleware\Widget\FeaturedProduct\FormMiddleware::class, 0);
    $mm->registerMiddleware(\KED\Module\Catalog\Middleware\Widget\ProductFilter\FormMiddleware::class, 0);
});
...
$eventDispatcher->addListener(
    ...
    function (&$fields, Container $container) {
        $fields['productFilterTool'] = [
            'type' => $container->get(\KED\Module\Catalog\Services\Type\ProductFilterToolType::class),
        ...
        $fields['productTierPrice'] = [
            'type' => Type::listOf($container->get(\KED\Module\Catalog\Services\Type\ProductTierPriceType::class)),
        ...
        $fields['potentialVariants'] = [
            'type' => Type::listOf($container->get(\KED\Module\Catalog\Services\Type\ProductType::class)),
    ...
    function (array $items) {
        return array_merge($items, [
            [
                "id" => "product_add_new",
                "sort_order" => 10,
                "url" => \KED\generate_url("product.create"),
                "title" => "New Product",
                "icon" => "cubes",
            ...
            [
                "id" => "catalog_products",
                "sort_order" => 10,
                "url" => \KED\generate_url("product.grid"),
                "title" => "Products",
                "icon" => "boxes",
                "parent_id" => "catalog"
            ],
            [
                ...
                "sort_order" => 20,
                "url" => \KED\generate_url("category.grid"),
                "title" => "Categories",
                "icon" => "tags",
                "parent_id" => "catalog"
            ],
            [
                ...
                "sort_order" => 30,
                "url" => \KED\generate_url("attribute.grid"),
                "title" => "Attributes",
                "icon" => "ruler-combined",
                "parent_id" => "catalog"
            ],
            [
                ...
                "sort_order" => 40,
                "url" => \KED\generate_url("attribute.group.grid"),
...
$eventDispatcher->addListener(
    ...
    function ($affectedRows, \KED\Services\Db\Processor $processor) use (&$productIds) {
        foreach ($affectedRows as $row) {
            $groupIds = [];
